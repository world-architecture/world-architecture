...
// POST { architect, country, language }
// Returns full voiceover script as JSON
...
Exactly 4 projects. wikiSearch MUST be in English. Choose the most visually stunning and iconic projects.`;
...
export default async function handler(req, res) {
  ...
  const userMsg = language === "Deutsch"
    ? `Schreibe das Voiceover-Skript auf DEUTSCH für: "${architect}" aus ${country}. Exakt 4 bekannteste, visuell eindrucksvollste Projekte. NUR JSON.`
    : `Write the voiceover script in ENGLISH for: "${architect}" from ${country}. Exactly 4 most famous, visually stunning projects. ONLY JSON.`;
  ...
  try {
    ...
    if (!parsed) return res.status(500).json({ error: "JSON parse failed", raw: raw.substring(0, 500) });
